/**
 * R.L.S. Public School - Secure Admissions Backend
 * 
...
const basicAuth = (req, res, next) => {
    ...
    if (!authHeader) {
        res.setHeader('WWW-Authenticate', 'Basic realm="R.L.S. Public School Staff Desk Terminal"');
    ...
    if (user === 'admin' && pass === 'rls_secure_pass_2026') {
        return next();
    } else {
        res.setHeader('WWW-Authenticate', 'Basic realm="R.L.S. Public School Staff Desk Terminal"');
...
async function triggerNotificationsWebhook(payload) {
    const webhookUrl = process.env.NOTIFICATION_WEBHOOK_URL || 'https://api.rlspublicschool.in/webhooks/admissions';
...
app.post('/api/admission/submit', (req, res) => {
    upload(req, res, async (err) => {
        ...
        try {
            ...
            const admissionNo = 'RLSPS-2026-' + Math.floor(100000 + Math.random() * 900000);
...
app.listen(PORT, () => {
    console.log(`✦ R.L.S. Public School Admissions backend active on port ${PORT}`);
